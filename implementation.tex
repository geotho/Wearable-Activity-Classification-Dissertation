\chapter{Implementation}
  ...
  \section{Activity analysis}
    ...
    \subsection{Computer use}
      \begin{figure}[!ht]
        \centering
        \includegraphics[width=1.0\textwidth]{2by2computeruse}
      ...
      Computer use is predominently typing or using a laptop trackpad while seated. There is very little movement in the phone, as the leg is stationary. The watch exhibits some periodic movement punctuated by periods of inactivity. This is presumably from typing a word and then pausing.
      
      The Fourier transform has quite a peak at just of 4 \si{Hz}. The periodicity of typing is probably higher than any other activity, and so extracting the frequency of maximum power will differentiate this activity well from others.
    \subsection{Walking}
      \begin{figure}
        \centering
        \includegraphics[width=1.0\textwidth]{2by2walking}
      ...
      Walking is perhaps the most periodic of all the activities investigated. There is a very strong peak at 2 Hz, a typical walking pace. The rest of the Fourier transform is relatively flat. A good distinguishing feature for between activies that exhibit strong periodicity is the spectral flatness measure, discussed in Section~\ref{sec:feature-extraction}.

    \subsection{Cycling}
      \begin{figure}
        \centering
        \includegraphics[width=1.0\textwidth]{2by2cycling}
      ...
      \end{figure}
      Cycling is not as consistently periodic as one might expect, though the phone exhibits some periodicity in the $1 \sim 2 \si{Hz}$ range.
      
      There is not much information to be gained 
    \subsection{Standing}
      % \begin{figure}
      %   \centering
      %   \includegraphics[width=1.0\textwidth]{2by2standing}
    ...
    \subsection{Playing fussball}
      \begin{figure}
        \centering
        \includegraphics[width=1.0\textwidth]{2by2fussball}
    ...
    \subsection{Stair climbing}
      \begin{figure}
        \centering
        \includegraphics[width=1.0\textwidth]{2by2stairs}
    ...
    \subsection{Machine learning}
      ...
      \subsubsection{Naive Bayes classifier}
        ...
        Mathematically we want to find $\mathbb{P}(y_k \mid X_i)$ for each label $y_k$ and each instance $\mathbf{X}_i$.
          ...
          \mathbb{P}(X_i) \text{ is constant with respect to the label} & \\
          &= \mathbb{P}(y_k) \mathbb{P}(x_1 \mid y_k) \mathbb{P}(x_2 \mid y_k) \cdots \mathbb{P}(x_j \mid y_k) &
          \text{Naive independence assumption} & \\
